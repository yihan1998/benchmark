...
inline int Client::SendRequest(KVRequest * request) {
    ...
    switch (workload_.NextOperation()) {
        ...
        case READMODIFYWRITE:
            ...
            break;
        default:
            throw utils::Exception("Request operation type is not recognized!");
    }
    assert(status >= 0);
    return (status == kOK);
}
...
inline int Client::ReceiveReply(KVReply * reply) {
    ...
    switch (reply->op) {
        ...
        case READMODIFYWRITE:
            ...
            break;
        default:
            std::cout << " op: " << reply->op << ", ret: " << reply->return_val << std::endl; 
            throw utils::Exception("Reply operation type is not recognized!");
    }
    assert(status >= 0);
    return (status == kOK);
}
...
inline int Client::HandleWriteEvent(struct conn_info * info) {
    ...
    int len = cetus_write(info->sockfd, (char *)&request, sizeof(request));
