...
class DB {
 public:
  ...
  ///
  virtual void Init(int key_len, int value_len) { }
  ///
  ...
  /// @param value The value in the record.
  /// @return Zero on success, a non-zero error code on error.
  ///
  virtual int Read(const std::string &table, const std::string &key, std::string &value) = 0;
  ///
  ...
  /// @param value The value to insert in the record.
  /// @return Zero on success, a non-zero error code on error.
  ///
