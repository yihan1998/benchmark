...
int HashtableDB::Update(const std::string &table, const std::string &key,
    ...
    if (!field_table) {
        ...
        field_table->Insert(kv_pair.first.c_str(), insert_value);
    } else {
        KVPair kv_pair = std::make_pair(key, value);
        const char * update_value = CopyString(kv_pair.second);
        const char * old = field_table->Update(kv_pair.first.c_str(), update_value);
        if (!old) {
            field_table->Insert(kv_pair.first.c_str(), update_value);
        } else {
            DeleteString(old);
        }
    }
