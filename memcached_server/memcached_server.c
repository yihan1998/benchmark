...
struct thread_arg app_thread_arg[MAX_CPUS];

long long request, reply;

struct timeval start, log_start, end;
int established_flag = 0;

long long sec_recv_bytes, sec_send_bytes;
...
int
accept_connection(int epfd, int sock) {
    int c;
...
int handle_read_event(int sockid) {
    ...
    char send_buff[1024];
    sprintf(send_buff, "Hello from server");

    int send_len = write(sockid, send_buff, buff_size);
...
int
close_connection(int epfd, int sockid) {
    //printf(" ========== [%s] ==========\n", __func__);
...
void * server_thread(void * arg) {
    ...
    sprintf(result_buff, " [%d] recv payload rate: %.2f(Mbps), recv request rate: %.2f, send payload rate: %.2f(Mbps), send reply rate: %.2f\n", 
    	                accept_num, (recv_bytes * 8.0) / (total_time * 1000 * 1000), request / (total_time * 1000), 
...
main(int argc, char ** argv) {
    ...
    for (int i = 0; i < argc; i++){
        ...
        char junk;
        char s[20];
        if (sscanf(argv[i], "--size=%llu%c", &n, &junk) == 1){
            buff_size = n;
			printf(" >> buff size: %d\n", buff_size);
	...
	app_thread_arg[0].thread_id = 0;
	app_thread_arg[0].core_id = core_id;
    
    if (pthread_create(&app_thread[0], NULL, server_thread, (void *)&app_thread_arg[0]) != 0) {
